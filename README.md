...
# 👉PanguSync 8.13 [下载地址](https://pan.baidu.com/s/1WesHaKGO7uQMhPNE-BTDmg?pwd=abcd#list/path=%2F)
...
- ♓️编辑界面【配置项】勾选【$\color{red}{源新增字段时，若目标端也存在该字段，将进行自动映射，无需重新编辑任务}$】后，可实现字段自动映射功能【源和目标需要同类型数据库】。如果源和目标异构，即大部分字段名称都不相同，建议不要勾选此项，避免源新增字段刚好和目标某个字段名称一样，就自动映射了，而这个字段可能你并不想同步它
...
- 🌿修改复合主键字段名称、删除复合主键、添加复合主键后，请重新运行任务
